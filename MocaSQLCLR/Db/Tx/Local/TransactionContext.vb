...
#Region " Method "
		...
		Public Sub Start() Implements ITransactionContext.Start
			Try
				...
				Debug.Print(String.Format("Begin Transaction [{2}:{3}]: {0}, {1}", _dbms.Setting.Database, level, Thread.CurrentThread.ManagedThreadId, Me.GetHashCode))
		...
		Public Sub Complete() Implements ITransactionContext.Complete
			Try
				...
				Try
					_tx.Commit()
					_conn.Close()
					Debug.Print(String.Format("Complete Transaction [{1}:{2}]: {0}", _dbms.Setting.Database, Thread.CurrentThread.ManagedThreadId, Me.GetHashCode))
				Catch ex As Exception
					Throw New Exceptions.MocaRuntimeException(ex)
				Finally
					_tx.Dispose()
		...
		Public Sub Rollback() Implements ITransactionContext.Rollback
			Try
				...
				Try
					_tx.Rollback()
					_conn.Close()
					Debug.Print(String.Format("Rollback Transaction [{1}:{2}]: {0}", _dbms.Setting.Database, Thread.CurrentThread.ManagedThreadId, Me.GetHashCode))
				Catch ex As Exception
					Throw New Exceptions.MocaRuntimeException(ex)
				Finally
					_tx.Dispose()

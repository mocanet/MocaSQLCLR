...
#Region " Implements "
		...
		Public Function Invoke(invocation As Aop.IMethodInvocation) As Object Implements Aop.IMethodInterceptor.Invoke
			...
			Try
				...
				Debug.Print(String.Format("TransactionScope Start[{1}:{2}, {3}, {4}] !! (Aspect:{0})", methodName, Thread.CurrentThread.ManagedThreadId, txs.TransactionContextHashCode, _scopeOption, txs.IsolationLevel))
				...
				If txs.RollbackStatus Then
					txs.Rollback()
					Debug.Print(String.Format("TransactionScope Rollback[{1}:{2}] !! (Aspect:{0})", methodName, Thread.CurrentThread.ManagedThreadId, txs.TransactionContextHashCode))
					Return rc
				End If
				...
				Debug.Print(String.Format("TransactionScope End[{1}:{2}] !! (Aspect:{0})", methodName, Thread.CurrentThread.ManagedThreadId, txs.TransactionContextHashCode))
				Return rc
			Catch ex As Exception
				If txs IsNot Nothing Then
					' ロールバック
					txs.Rollback()
					Debug.Print(String.Format("TransactionScope Aborted[{1}:{2}] Exception[{3}]!! (Aspect:{0})", methodName, Thread.CurrentThread.ManagedThreadId, txs.TransactionContextHashCode, ex.Message))
				Else
					Debug.Print(String.Format("TransactionScope Aborted[{1}:{2}] Exception[{3}]!! (Aspect:{0})", methodName, Thread.CurrentThread.ManagedThreadId, String.Empty, ex.Message))
				End If
				Throw ex
			Finally
				If txs IsNot Nothing Then

		...
		Public Function Invoke(ByVal invocation As Aop.IMethodInvocation) As Object Implements Aop.IMethodInterceptor.Invoke
			...
			Try
				...
				Using txs As TransactionScope = _newTransactionScope()
					...
					End If
					Debug.WriteLine(String.Format("Transaction Start[{1}, {2}, {3}] !! (Aspect:{0})", _methodName, txLocalIdentifier, _scopeOption, _isolationLevel))
			...
			Catch ex As Exception
				Debug.WriteLine(String.Format("Transaction   Aborted Exception[{1}] !! (Aspect:{0})", _methodName, ex.Message))
				Throw ex
			End Try
...
#Region " Method "
		...
		Protected Sub TransactionManager_DistributedTransactionStarted(ByVal sender As Object, ByVal d As TransactionEventArgs)
			Debug.WriteLine(String.Format("MS-DTC トランザクションへの昇格が発生しました。 (Aspect:{0})", _methodName))
		End Sub
		...
		Private Sub TransactionCompleted_OnCommitted(sender As Object, e As System.Transactions.TransactionEventArgs)
			Debug.WriteLine(String.Format("Transaction   {2} [{1}] !! (Aspect:{0})", _methodName, e.Transaction.TransactionInformation.LocalIdentifier, e.Transaction.TransactionInformation.Status.ToString))
		End Sub
